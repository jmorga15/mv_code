...
def fn_of_koff(arg):
    ...
    indexing_array = np.logspace(-2, 1.0, num=40)
...
#    i = 10
    ...
    for i in antigen_array:
        ...
        while j < 101:
            #N=55#
            (accumulator, tc, X_max, mv_lt) = mv_sim_num_direct( i, k_off, 100, 10 )
            
            #stab_conc_mean_sim[0,m] = stab_conc_mean_sim[0,m] +  uconc
            
            #lt_bin_sim[m] = lt_bin_sim[m] + lt_bin[0]
            mv_int = mv_int + np.sum(mv_lt)
            len_track = len_track + len(mv_lt)
            ...
            j = j+1
            #print(j)
        mv_int = mv_int / len_track
        mv_lt_mat[m,0] = mv_int
   ...
   ### Write data to HDF5
    ...
    np.save('lt_ave'+ str(koff_index) + '.npy', mv_lt_mat)
